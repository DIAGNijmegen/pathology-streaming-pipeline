...
class StreamingCNN(object):
    ...
    def forward(self, image):
        ...
        if self.gather_input_gradient:
            self.saliency_map = torch.zeros(image.shape, dtype=self.dtype)
    ...
    def backward(self, image, grad):
        ...
        for row in iterator:
            for col in range(n_cols):
                ...
                tile = tile.cuda()
                if self.gather_input_gradient: tile.requires_grad = True
                ...
                trimmed_grad = trimmed_grad.cuda()
    ...
    def _backward_gather_statistics_hook(self, module, grad_in, grad_out):
        ...
        if grad_in[0] is not None:
            ...
            # return ((new_grad_in.type(self.dtype) * 2 - 1), *grad_in[1:])
    ...
    def _backward_streaming_hook(self, module, grad_in, grad_out):
        ...
        if new_output_box.height > 0 and new_output_box.width > 0:
            ...
            if self.gather_input_gradient:
                if module.in_channels == 3:
                    valid_grad_in = grad_in[0][:, :, 
                                               lost_top*stride:grad_in[0].shape[2] - lost_bottom*stride,
                                               lost_left*stride:grad_in[0].shape[3] - lost_right*stride]

                    relevant_input_grad = valid_grad_in[:, :, 
                                                        new_output_box.y*stride:
                                                        new_output_box.y*stride + new_output_box.height*stride,
                                                        new_output_box.x*stride:
                                                        new_output_box.x*stride + new_output_box.width*stride]

                    self.saliency_map[:, :,
                                      updated_total_indices.y * stride:
                                      updated_total_indices.height * stride,
                                      updated_total_indices.x * stride - relevant_input_grad.shape[3]:
                                      updated_total_indices.x * stride] = relevant_input_grad.detach().cpu()
            ...
            if relevant_grad.dtype != relevant_input.dtype:
                relevant_input = relevant_input.to(relevant_grad.dtype)

            # print(sides, relevant_grad.shape, relevant_input.shape, module.weight.shape, inpt[0].shape, input_y)
        ...
        if len(grad_in) == 3:
            if module.bias is None:
                return grad_in[0], grad_weight, None
            else: 
                grad_bias = grad_bias.to(grad_weight.dtype)
                return grad_in[0], grad_weight, grad_bias
        elif len(grad_in) == 2:
