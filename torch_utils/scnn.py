...
class Lost:
    ...
    def __str__(self):
        return '(Lost front:%2.1f top:%2.1f left:%2.1f back:%2.f bottom:%2.1f right:%2.1f)' \
            % (self.front, self.top, self.left, self.back, self.bottom, self.right)
...
class StreamingConv2dF(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:
            ...
            if weight.shape[-1] == 1 or groups > 1 or stride[0] > 1:
                ...
                grad_in = cpp_functions.backward_input(inpt.shape, grad_output, weight.to(inpt.dtype), stride, 
                                                      padding, dilation, groups, True, False)
        else:
            grad_in = None

        grad = grad_output

        lost_front = grad_lost.front if not sides.front else 0
        lost_back = grad_lost.back if not sides.back else 0
        ...
        if ctx._3d:
            valid_grad = grad[:, :,
                              lost_front:grad.shape[Z_DIM] - lost_back,
        ...
        # Update inplace
        seen_indices.z = updated_total_indices.z
        seen_indices.depth = updated_total_indices.depth
        ...
        if new_output_box.height > 0 and new_output_box.width > 0 and new_output_box.depth > 0:
            if ctx._3d:
                relevant_grad = valid_grad[:, :, new_output_box.z:new_output_box.z + new_output_box.depth,
            ...
            input_z = (new_output_box.z + lost_front) * stride[0]
            ...
            input_y -= padding[1]
            input_x -= padding[2]
            input_z = max(0, input_z)
            input_x = max(0, input_x)
            input_y = max(0, input_y)

            if ctx._3d: relevant_input_depth = relevant_grad.shape[Z_DIM] * stride[0] + (kernel_size[0] - 1)
            else: relevant_input_depth = 0
            ...
            if ctx._3d:
                relevant_input = inpt[:, :,
                                      input_z:input_z + relevant_input_depth,
            ...
            if (padding[0] > 0 or padding[1] > 0 or padding[2] > 0) and \
                    (sides.back or sides.front or sides.top or sides.left or sides.right or sides.bottom):
                # The size of the tile should remain equal.
                crop_back = padding[0] if sides.front else 0
                crop_bottom = padding[1] if sides.top else 0
                crop_right = padding[2] if sides.left else 0
                if ctx._3d:
                    relevant_input = inpt[:, :,
                                          input_z:input_z + relevant_input_depth - crop_back,
                ...
                if ctx._3d:
                    relevant_input = torch.nn.functional.pad(relevant_input, [padding[2] if sides.front else 0,
                                                                              padding[2] if sides.back else 0,
                                                                              padding[1] if sides.left else 0,
                                                                              padding[1] if sides.right else 0,
                                                                              padding[0] if sides.top else 0,
                                                                              padding[0] if sides.bottom else 0])
                else:
            ...
            grad_weight = cpp_functions.backward(weight.shape,
                                                ...
                                                (0, 0, 0) if ctx._3d else (0, 0),  # padding
                                                stride if ctx._3d else stride[1:3], dilation, groups,
                                                torch.backends.cudnn.benchmark,  # benchmark
                                                torch.backends.cudnn.deterministic)  # deterministic

            if bias is not None:
                if ctx._3d: grad_bias = relevant_grad[0].sum((1, 2, 3))
                else: grad_bias = relevant_grad[0].sum((1, 2))
...
class StreamingConv2d(_ConvNd):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True, padding_mode='zeros'):
        ...
        self.grad_lost = Lost(0, 0, 0, 0, 0, 0)
        self.tile_output_box = Box(0, 0, 0, 0, 0, 0, None)
        self.reset()

    def reset(self):
        self.seen_indices = Box(0, 0, 0, 0, 0, 0, None)
        self.input_loc = Box(0, 0, 0, 0, 0, 0, None)
...
class StreamingCNN(object):
    ...
    def __init__(self, stream_module, tile_shape, verbose=False, deterministic=False,
                 ...
                 normalize_on_gpu=True, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225],
                 state_dict=None):
        '''
        Parameters:
    ...
    def _gather_backward_statistics(self, tile):
        ...
        if self._3d:
            gradient[:, :,
                     self.tile_output_lost.front:output.shape[Z_DIM] - self.tile_output_lost.back,
    ...
    def _non_max_border_amount(self, tensor):
        ...
        if self._3d:
            front, top, left = non_zero.min(dim=0)[0]
            back, bottom, right = torch.tensor([*tensor.size()], dtype=torch.long, device=self.device) - non_zero.max(dim=0)[0] - 1
            return Lost(front, back, top, left, bottom, right)
        else:
            ...
            return Lost(0, 0, top, left, bottom, right)
    ...
    def forward(self, image):
        ...
        tile_depth = self.tile_shape[Z_DIM] if self._3d else 1
        ...
        valid_output_depth = 1
        if self._3d:
            valid_output_depth = (self._tile_output_shape[Z_DIM] - self.tile_output_lost.front - self.tile_output_lost.back)

        # We will keep track which part of the output of the whole image we
        # already filled with valid values from tile output.
        already_filled = Box(0, 0, 0, 0, 0, 0, None)
        ...
        output_depth = 1
        if self._3d:
            output_depth = (image.shape[Z_DIM] - self.tile_shape[Z_DIM]) // self.output_stride[0] + self._tile_output_shape[Z_DIM]

        output_shape = IOShape(1, self._tile_output_shape[1], output_depth, output_height, output_width)
        if self._3d:
        ...
        n_slices = math.ceil(float(output_depth) / float(valid_output_depth))
        n_rows = math.ceil(output_height.item() / valid_output_height.item())
        n_cols = math.ceil(output_width.item() / valid_output_width.item())

        if image.shape[W_DIM] <= tile_width: n_cols = 1
        if image.shape[H_DIM] <= tile_height: n_rows = 1
        if image.shape[Z_DIM] <= tile_depth: n_slices = 1

        if self.gather_input_gradient:
            self.saliency_map = torch.zeros(image.shape, dtype=self.dtype, device=self.device)

        if self.verbose: print('Number of tiles in forward:', n_slices * n_rows * n_cols)
        if self.verbose: iterator = tqdm(range(n_slices))
        else: iterator = range(n_slices)
        ...
        with torch.no_grad():
            for sli in iterator:
                for row in range(n_rows):
                    for col in range(n_cols):
                        ...
                        output_z = sli * valid_output_depth
                        ...
                        sides_front = True if sli == 0 else False
                        sides_top = True if row == 0 else False
                        sides_left = True if col == 0 else False

                        if self._3d: sides_back = True if output_z * self.output_stride[0] + self.tile_shape[Z_DIM] >= image.shape[Z_DIM] else False
                        else: sides_back = True
                        ...
                        sides = Sides(sides_front, sides_back, sides_left, sides_top, sides_right, sides_bottom)
                        ...
                        if sides_back:   output_z = (image.shape[Z_DIM] - self.tile_shape[Z_DIM]) // self.output_stride[0]
                        ...
                        output_z = output_z if not sides.front else 0
                        output_y = output_y if not sides.top else 0
                        output_x = output_x if not sides.left else 0
                        output_loc = Box(output_z + lost.front, 0, output_y + lost.top, -1, output_x + lost.left, -1, sides)

                        # Coordinates of the input w.r.t. the output of full image
                        tile_z = output_z * self.output_stride[0]
                        ...
                        if self._3d:
                            trimmed_output = tile_output[:, :,
                                                         lost.front:tile_output.shape[Z_DIM] - lost.back,
                        ...
                        if self._3d:
                            relevant_output = trimmed_output[:, :,
                                                             new_output_box.z:updated_total_indices.z + new_output_box.depth,
                                                             new_output_box.y:updated_total_indices.y + new_output_box.height,
                                                             new_output_box.x:new_output_box.x + new_output_box.width]
                            output[:, :,
                                   int(updated_total_indices.z):int(updated_total_indices.depth),
            ...
            assert sides_bottom and sides_right and sides_back, "It seems like we could not reconstruct all output"  #type:ignore
    ...
    def backward(self, image, grad):
        ...
        n_slices = math.ceil(float(depth - grad_lost.front - grad_lost.back) / float(valid_grad_depth))
        if not self._3d: n_slices = 1
        ...
        print('Number of tiles in backprop:', n_slices * n_rows * n_cols)
        if self.verbose: print('Number of tiles in backprop:', n_slices * n_rows * n_cols)
        if self.verbose: sli_iterator = tqdm(range(n_slices))
        else: sli_iterator = range(n_slices)

        for sli in sli_iterator:
            for row in range(n_rows):
                for col in range(n_cols):
                    # Since we determine output (gradient) coordinates based on input
                    # coordinates. We need to divide by output stride.
                    output_z = sli * valid_grad_depth // self.output_stride[0]
                    ...
                    if self._3d: sides_back = True if output_z + output_depth >= grad.shape[Z_DIM] else False
                    else: sides_back = True
                    ...
                    if self._3d:
                        trimmed_grad = gradient[:, :,
                                                lost.front:gradient.shape[Z_DIM] - lost.back,
        ...
        assert sides_right and sides_bottom and sides_back, "It seems like we could not reconstruct all output"  # type:ignore

    def _get_tile_lost_for_sides(self, sides):
        lost_front = self.tile_output_lost.front if not sides.front else 0
        lost_back = self.tile_output_lost.back if not sides.back else 0
        ...
        lost = Lost(lost_front, lost_back, lost_top, lost_left, lost_bottom, lost_right)
        return lost
    ...
    def _add_hooks(self, forward_hook, backward_hook,
                   forward_modules=(torch.nn.Conv2d, torch.nn.MaxPool2d, torch.nn.AvgPool2d,
                                    torch.nn.Conv3d, torch.nn.MaxPool3d, torch.nn.AvgPool3d),
                   back_modules=(torch.nn.Conv2d, torch.nn.MaxPool2d,
                                 torch.nn.Conv3d, torch.nn.MaxPool3d)):
        for mod in self.stream_module.modules():
    ...
    def _forward_gather_statistics_hook(self, module, inpt, output):
        ...
        if not torch.is_grad_enabled():  # type:ignore
            ...
            if isinstance(module, (torch.nn.MaxPool2d, torch.nn.MaxPool3d)) or \
                ...
                if module.padding != 0:
                    ...
                    if not isinstance(module.padding, tuple):
                        if self._3d: padding = [module.padding, module.padding, module.padding]
                        else: padding = [module.padding, module.padding]
                    if self._3d:
                        padded_input = torch.nn.functional.pad(inpt[0], [padding[0], padding[0], padding[0], padding[0], padding[0], padding[0]])
                    else:
                        padded_input = torch.nn.functional.pad(inpt[0], [padding[1], padding[1], padding[0], padding[0]])
                else:
                    padded_input = inpt[0]

                if self._3d: new_output = torch.nn.functional.avg_pool3d(padded_input, kernel_size, stride)
                else: new_output = torch.nn.functional.avg_pool2d(padded_input, kernel_size[1:], stride[1:])
            ...
            output.fill_(0)
            if self._3d:
                output[:,:,lost.front:output[0, 0].shape[0] - lost.back,
                       lost.top:output[0, 0].shape[1] - lost.bottom,
                       lost.left:output[0, 0].shape[2] - lost.right] = 1
            else:
    ...
    def _backward_gather_statistics_hook(self, module, grad_in, grad_out):
        ...
        if grad_in[0] is not None:
            ...
            if isinstance(module, (torch.nn.MaxPool3d, torch.nn.MaxPool2d)):
                ...
                if self._3d: new_outpt = torch.nn.functional.avg_pool3d(padded_inpt, kernel_size, stride)[0]
                else: new_outpt = torch.nn.functional.avg_pool2d(padded_inpt, kernel_size[1:], stride[1:])[0]
                ...
                if self._3d: f_grad = np.repeat(f_grad, stride[2], axis=2)
                grad = np.zeros(grad_in[0].shape[2:])
                if self._3d: grad[:f_grad.shape[0], :f_grad.shape[1], :f_grad.shape[2]] = f_grad
                else: grad[:f_grad.shape[0], :f_grad.shape[1]] = f_grad
            ...
            if (stride[0] > 1 and kernel_size[0] > stride[0]) or \
                ...
                overlap_2 = kernel_size[2] - stride[2]
                if self._3d:
                    valid_grad[valid_lost.front + overlap_0:
                               valid_grad.shape[0] - valid_lost.back - overlap_0,
                               valid_lost.top + overlap_1:
                               valid_grad.shape[1] - valid_lost.bottom - overlap_1,
                               valid_lost.left + overlap_2:
                               valid_grad.shape[2] - valid_lost.right - overlap_2] = 1
                else:
    ...
    def _backward_saliency_hook(self, module: StreamingConv2d, grad_in, grad_out, is_bias=False, change_grad=True):
        ...
        if module.in_channels == 3:
            if self._3d:
                # missing *stride in 3D implementation
                valid_grad_in = grad_in[0][:, :,
                                           lost.front:grad_in.shape[Z_DIM] - lost_back,
                                           lost.top:grad_in.shape[H_DIM] - lost_bottom,
                                           lost.left:grad_in.shape[W_DIM] - lost_right]

                relevant_input_grad = valid_grad_in[:, :, 
                                                    new_output_box.z:new_output_box.z + new_output_box.depth,
                                                    new_output_box.y:new_output_box.y + new_output_box.height,
                                                    new_output_box.x:new_output_box.x + new_output_box.width]
                self.saliency_map[:, :,
                                  updated_total_indices.z:
                                  updated_total_indices.depth,
                                  updated_total_indices.y:
                                  updated_total_indices.height,
                                  updated_total_indices.x - relevant_input_grad.shape[W_DIM]:
    ...
    def _new_value_indices(data_shape, data_indices, old_value_indices):
        ...
        """
        rel_front, rel_back, rel_top, rel_bottom, rel_left, rel_right = 0, 0, 0, 0, 0, 0

        old_values_z = old_value_indices.z
        old_values_y = old_value_indices.y
        old_values_x = old_value_indices.x
        old_values_depth = old_value_indices.depth
        ...
        # Check if new slice
        if data_indices.y == 0 and data_indices.x == 0:
            old_values_z = old_values_depth
            old_values_depth = data_indices.z + data_shape[Z_DIM]
            old_values_y = 0
            old_values_height = data_shape[H_DIM]
        ...
        # Check z-axis:
        # Equal to depth logic (see above)
        if len(data_shape) == 5:
            if data_indices.z == old_values_z:
                rel_front = 0
                rel_back = data_shape[Z_DIM]
            else:
                assert old_values_z - data_indices.z >= 0, "We miss data in z-axis"
                rel_front = old_values_z - data_indices.z
                rel_back = data_shape[Z_DIM]
        else:
            rel_back = 1
        ...
        assert rel_front >= 0, f"We miss data in z-axis before: {data_indices}"

        new_value_indices = Box(rel_front, rel_back - rel_front, rel_top, rel_bottom - rel_top, rel_left, rel_right - rel_left, None)
        old_value_indices = Box(int(old_values_z), int(old_values_depth), int(old_values_y), int(old_values_height), int(old_values_x), 0, None)
